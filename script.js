        ...
        (function(){
          var BIN_ID  = '6a0830f9c0954111d82ff1b9';
          var API_KEY = '$2a$10$XTuUrSyyZE8ZGNEJ5a5u8u6v/oRcBuEI50346uVqVm3DZ.WdPjuE.';
          var BASE    = 'https://api.jsonbin.io/v3/b/' + BIN_ID;
          var HEADERS = { 'Content-Type': 'application/json', 'X-Master-Key': API_KEY };

          // liked = apakah SESSION INI sudah like (belum di-save ke jsonbin sebagai "unlike")
          // Kita pakai sessionStorage supaya per tab, bukan per browser
          var liked = false;
          var count = 1;
          var busy  = false;
          ...
          function saveToJsonBin(newCount) {
            return fetch(BASE, {
              method: 'PUT',
              headers: HEADERS,
          ...
          fetch(BASE + '/latest', { headers: { 'X-Master-Key': API_KEY } })
            .then(r => r.json())
            .then(j => {
              count = j.record.likes || 1;
              cntEl.textContent = count;
              // Cek apakah user ini sudah like di session ini
              if (sessionStorage.getItem('neysa_liked') === '1') {
                applyLikedUI(true);
              }
            })
            .catch(() => { cntEl.textContent = count; });
          ...
          window.toggleProfileLike = function(){
            ...
            // Kalau mau unlike tapi session ini belum pernah like → abaikan
            if (!willLike && sessionStorage.getItem('neysa_liked') !== '1') {
              busy = false;
              return;
            }

            var newCount = willLike ? count + 1 : Math.max(1, count - 1);
            ...
            // Simpan ke JSONBin
            saveToJsonBin(newCount)
              .then(saved => { count = saved; cntEl.textContent = count; })
              .catch(() => {})
              .finally(() => { busy = false; });
          };
        })();
...
/* ============================= */
  ...
  /* ── VIEW COUNTER (global via JSONBin) ── */
  (function(){
    const BIN_ID  = '6a083590c0954111d83006e3';
    const API_KEY = '$2a$10$XTuUrSyyZE8ZGNEJ5a5u8u6v/oRcBuEI50346uVqVm3DZ.WdPjuE.';
    const BASE    = 'https://api.jsonbin.io/v3/b/' + BIN_ID;
    const HEADERS = { 'Content-Type': 'application/json', 'X-Master-Key': API_KEY };
    const el      = document.getElementById('dc-view-count');

    fetch(BASE + '/latest', { headers: { 'X-Master-Key': API_KEY } })
      .then(r => r.json())
      .then(j => {
        let views = j.record.views || 0;
        if (!sessionStorage.getItem('neysa_viewed')) {
          ...
          fetch(BASE, {
            method: 'PUT',
            headers: HEADERS,
            body: JSON.stringify({ likes: j.record.likes || 0, views: views })
          }).catch(() => {});
        }
